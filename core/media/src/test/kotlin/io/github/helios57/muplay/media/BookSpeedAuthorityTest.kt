...
class BookSpeedAuthorityTest {
  ...
  private fun controllerSource(): File =
    File(repoRoot(), "core/media/src/main/kotlin/io/github/helios57/muplay/media/BookSpeedController.kt")
    ...
    assertThat(itemGrainNames.filter { text.contains(it) })
      .describedAs(
        "item-grain names in BookSpeedController.kt -- a book's speed is the book's " +
          "(io.github.helios57.muplay.model.BookSettings), and a second reader is a book with two speeds",
      )
      .isEmpty()
  }
  ...
  fun `the needles this rule looks for are the ones that really reach that table`() {
    ...
    val writer = File(repoRoot(), "core/media/src/main/kotlin/io/github/helios57/muplay/media/ProgressWriter.kt")
