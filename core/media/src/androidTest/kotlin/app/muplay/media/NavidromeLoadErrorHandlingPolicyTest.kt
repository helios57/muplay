...
class NavidromeLoadErrorHandlingPolicyTest {
  ...
  fun theResponseCodeIsReadOutOfTheExceptionAndNotAssumed() {
    ...
    // both: ours says 8_000 at attempt 4, Media3's says 3_000.
    assertThat(policy.getRetryDelayMsFor(loadError(responseCode = 429, errorCount = 4)))
      .isEqualTo(8_000L)
    assertThat(policy.getRetryDelayMsFor(loadError(responseCode = 503, errorCount = 4)))
      .isEqualTo(3_000L)
  }
  ...
  private fun loadError(
    ...
    headers: Map<String, List<String>> = retryAfter?.let { mapOf("Retry-After" to listOf(it)) }.orEmpty(),
