...
class MuPlayDataSourceFactoryTest {
  ...
  fun setUp() {
    ...
    //
    // The cache is this test's own (see above), so the *only* production collaborator swapped out
    // here is the `Call.Factory`, which has to be the marker one for `server.requestCount` to mean
    // anything.
