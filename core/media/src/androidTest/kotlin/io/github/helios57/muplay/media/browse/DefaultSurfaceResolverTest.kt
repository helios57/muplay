...
import io.github.helios57.muplay.model.browse.BrowseSurfaces
 ...
 * false.
 *
 * Note the package this app answers to here is **not** `io.github.helios57.muplay`: a library module's
 * instrumented tests are self-instrumenting, so `context.packageName` is `io.github.helios57.muplay.media.test`.
...
class DefaultSurfaceResolverTest {
  ...
  fun theSameHintFromAnyOtherPackageIsRefused() {
    ...
    val surfaces = listOf(ownPackage, "$ownPackage.evil", "com.example.other", "io.github.helios57.muplay")
    ...
    assertThat(surfaces).containsExactly(
      ...
      BrowseSurface.PHONE,
      BrowseSurface.PHONE,
      // `io.github.helios57.muplay` is the *shipping* application id and is deliberately NOT this process's
