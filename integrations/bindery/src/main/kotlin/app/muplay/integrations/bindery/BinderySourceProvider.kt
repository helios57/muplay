...
/**
 * Builds a [BinderySource] from whatever is currently configured — or reports that nothing is.
 *
 ...
 * Bindery is a *normal* app — it is the state most users are in, permanently — so "not configured"
 ...
 * that every test configures around is a path no test exercises. `BinderySourceProviderTest` is
 ...
 *
 * The [IntegrationService.LIDARR] entry is never consulted here. That is not an omission: a
 * Bindery provider that asked for Lidarr's credentials would be the cross-service reachability the
 * severability contract's third clause forbids — and this class is the mirror image of
 * `LidarrSourceProvider`, in a module that does not depend on the one that holds it.
 */
@Singleton
class BinderySourceProvider @Inject constructor(
  private val credentialStore: IntegrationCredentialStore,
  private val factory: BinderySourceFactory,
) {

  /**
   * The configured Bindery, or `null` when there is none.
   *
   * `as?` rather than a cast: [IntegrationCredentialStore.load] is typed to the sealed supertype,
   * and a Lidarr credential arriving under `BINDERY` is a corrupt store, not a crash.
   */
  suspend fun current(): BinderySource? =
    (credentialStore.load(IntegrationService.BINDERY) as? IntegrationCredentials.Bindery)
      ?.let(factory::create)
}
