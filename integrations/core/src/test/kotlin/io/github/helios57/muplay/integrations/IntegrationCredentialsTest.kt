package io.github.helios57.muplay.integrations
...
class IntegrationCredentialsTest {
  ...
  fun `the two services use two different keystore aliases`() {
    assertThat(IntegrationService.entries.map { IntegrationCredentialStore.keyAlias(it) })
      .containsExactly("io.github.helios57.muplay.integrations.lidarr", "io.github.helios57.muplay.integrations.bindery")
  }

  /**
   * The aliases are the app's own namespace and are not the one holding the Navidrome password.
   * `CredentialStore.clear()` destroys `io.github.helios57.muplay.credentials`; an integration sharing it would
  ...
  fun `no integration shares the navidrome credential alias`() {
    assertThat(IntegrationService.entries.map { IntegrationCredentialStore.keyAlias(it) })
      .allSatisfy { assertThat(it).startsWith("io.github.helios57.muplay.integrations.") }
      .doesNotContain("io.github.helios57.muplay.credentials")
  }
